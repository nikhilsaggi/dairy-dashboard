const TaskDao = require("../models/TaskDao");
 ...
 class TaskList {
   ...
   async showEnvData(req, res) {
    const querySpec = {
      query: "SELECT  TOP 14 c.datets FROM c ORDER BY c.datets DESC"
   ...
   async showTasks(req, res) {
    ...
    for(var i = 0; i < ARRAYS.length; i ++){
      const query = "SELECT c.DIM, c.Protein, c.Yieldgr, c.Fat, c.Blood, c.datets  FROM c WHERE c.AnimalID =" + ARRAYS[i] + "ORDER BY c.datets DESC"
      const querySpec = {
        query: query
      };
      const items = await this.taskDao.find(querySpec);
